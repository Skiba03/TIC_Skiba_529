...
def main():
    ...
    for sequence in original_sequences:
        ...
        file.write('\n')
        file.close()
        encoded_data_ac, encoded_sequence_ac = encoded_ac(unique_chars, probability, sequence_alphabet_size, sequence)
        bps_ac = len(encoded_sequence_ac)/sequence_length
        decoded_sequence_ac = decode_ac(encoded_data_ac, sequence_length)
        encoded_data_hc, encoded_sequence_hc = encode_ch(unique_chars, probability, sequence)
        bps_hc = len(encoded_sequence_hc)/sequence_length
        decoded_sequence_hc = decode_ch(encoded_data_hc, sequence)
        file = open("results_AC_CH.txt", "a")
        file.write('___________Арифметичне кодування______________' + '\n')
        ...
        file.write('\n')
        file.write('___________Кодування Хаффмана______________' + '\n')
...
def encoded_ac(uniq_chars, probabilitys, alphabet_size, sequence):
...
def float_bin(point, size_cod):
    ...
    for x in range(size_cod):
        point = point * 2
        if point > 1:
            binary_cod = binary_cod + str(1)
            x = int(point)
            point = point - x
        elif point < 1:
            binary_cod = binary_cod + str(0)
        elif point == 1:
            binary_cod = binary_cod + str(1)
            break
    return binary_cod
...
def decode_ch(encoded_sequence, sequence):
    ...
    for i in range(len(encode)):
        for j in range(len(symbol_code)):
            if encode[i] == symbol_code[j][1]:
                sequence = sequence + str(symbol_code)
                flag = 1
        if flag == 1:
            flag = 0
        else:
